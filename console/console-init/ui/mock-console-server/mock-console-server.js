...
const availableAddressSpacePlans = [
  {
    ...
    Spec: {
      ...
      AddressPlans: availableAddressPlans.filter(
        p => !p.ObjectMeta.Name.startsWith("brokered-")
      ),
      DisplayName: "Small",
      ShortDescription:
  ...
  },
  {
    ...
    Spec: {
      ...
      AddressPlans: availableAddressPlans.filter(p =>
        p.ObjectMeta.Name.startsWith("brokered-")
      ),
...
function createAddressSpace(as) {
  var namespace = availableNamespaces.find(
    n => n.ObjectMeta.Name === as.ObjectMeta.Namespace
  );
  ...
  var spacePlan = availableAddressSpacePlans.find(
    o => o.ObjectMeta.Name === as.Spec.Plan
  );
  ...
  if (
    addressSpaces.find(
      existing =>
        as.ObjectMeta.Name === existing.ObjectMeta.Name &&
        as.ObjectMeta.Namespace === existing.ObjectMeta.Namespace
    ) !== undefined
  ) {
  ...
  var addressSpace = {
    ObjectMeta: {
      ...
      Uid: uuidv1(),
      CreationTimestamp: as.ObjectMeta.CreationTimestamp
        ? as.ObjectMeta.CreationTimestamp
...
function patchAddressSpace(objectMeta, jsonPatch, patchType) {
  var index = addressSpaces.findIndex(
    existing =>
      objectMeta.Name === existing.ObjectMeta.Name &&
      objectMeta.Namespace === existing.ObjectMeta.Namespace
  );
  if (index < 0) {
    ...
    if (replacement.Plan !== current.Plan) {
      var replacementPlan =
        typeof replacement.Plan === "string"
          ? replacement.Plan
          : replacement.ObjectMeta.Name;
      var spacePlan = availableAddressSpacePlans.find(
        o => o.ObjectMeta.Name === replacementPlan
      );
      if (spacePlan === undefined) {
        var knownPlansNames = availableAddressSpacePlans.map(
          p => p.ObjectMeta.Name
...
function createConnection(addressSpace, hostname) {
  ...
  return {
    ObjectMeta: {
      ...
      CreationTimestamp: getRandomCreationDate(
        addressSpace.ObjectMeta.CreationTimestamp
      )
    },
    Spec: {
...
addressSpaces.forEach(as => {
  addressspace_connection[as.ObjectMeta.Uid] = connections.filter(
    c => c.Spec.AddressSpace.ObjectMeta.Uid === as.ObjectMeta.Uid
  );
});
...
function createAddress(addr) {
  var namespace = availableNamespaces.find(
    n => n.ObjectMeta.Name === addr.ObjectMeta.Namespace
  );
  if (namespace === undefined) {
    var knownNamespaces = availableNamespaces.map(p => p.ObjectMeta.Name);
  ...
  var addressSpacesInNamespace = addressSpaces.filter(
    as => as.ObjectMeta.Namespace === addr.ObjectMeta.Namespace
  );
  var addressSpace = addressSpacesInNamespace.find(
    as => as.ObjectMeta.Name === addr.Spec.AddressSpace
  );
  if (addressSpace === undefined) {
    var addressspacenames = addressSpacesInNamespace.map(
      p => p.ObjectMeta.Name
    );
  ...
  var plan = availableAddressPlans.find(
    o => o.ObjectMeta.Name === addr.Spec.Plan
  );
  if (plan === undefined) {
  ...
  if (
    addresses.find(
      existing =>
        addr.ObjectMeta.Name === existing.ObjectMeta.Name &&
        addr.ObjectMeta.Namespace === existing.ObjectMeta.Namespace
    ) !== undefined
  ) {
  ...
  var address = {
    ObjectMeta: {
      ...
      Uid: uuidv1(),
      CreationTimestamp: addr.ObjectMeta.CreationTimestamp
        ? addr.ObjectMeta.CreationTimestamp
        : getRandomCreationDate()
    },
    Spec: {
  ...
  if (patched.newDocument) {
    ...
    if (replacement.Plan !== current.Plan) {
      var replacementPlan =
        typeof replacement.Plan === "string"
          ? replacement.Plan
          : replacement.Plan.ObjectMeta.Name;
...
function deleteAddress(metadata) {
  var index = addresses.findIndex(
    existing =>
      metadata.Name === existing.ObjectMeta.Name &&
      metadata.Namespace === existing.ObjectMeta.Namespace
  );
  if (index < 0) {
...
function closeConnection(objectmeta) {
  ...
  if (index < 0) {
    var knownCons = connections
      .filter(c => c.ObjectMeta.Namespace === objectmeta.Namespace)
      .map(c => c.ObjectMeta.Name);
...
function* makeAddrIter(namespace, addressspace) {
  var filter = addresses.filter(
    a =>
      a.ObjectMeta.Namespace === namespace &&
      a.ObjectMeta.Name.startsWith(addressspace + ".")
  );
  var i = 0;
  while (filter.length) {
...
var addressItrs = {};
addressSpaces.forEach(as => {
  addressItrs[as.ObjectMeta.Uid] = makeAddrIter(
    as.ObjectMeta.Namespace,
    as.ObjectMeta.Name
  );
});
...
`;
    ...
    addressPlans: (parent, args, context, info) => {
      if (args.addressSpacePlan === undefined) {
        ...
        var spacePlan = availableAddressSpacePlans.find(
          o => o.ObjectMeta.Name === args.addressSpacePlan
        );
        if (spacePlan === undefined) {
          var knownPlansNames = availableAddressSpacePlans.map(
            p => p.ObjectMeta.Name
  ...
  AddressSpace_consoleapi_enmasse_io_v1beta1: {
    Connections: (parent, args, context, info) => {
      ...
      var as = parent;
      var cons =
        as.ObjectMeta.Uid in addressspace_connection
          ? addressspace_connection[as.ObjectMeta.Uid]
          : [];
    ...
    Addresses: (parent, args, context, info) => {
      ...
      var addrs = addresses
        .filter(
          a =>
            as.ObjectMeta.Namespace === a.ObjectMeta.Namespace &&
  ...
  Address_consoleapi_enmasse_io_v1beta1: {
    ...
    Links: (parent, args, context, info) => {
      ...
      var addrlinks = links
        .filter(
          l =>
            l.Spec.Connection.Spec.AddressSpace.ObjectMeta.Namespace ===
              addr.ObjectMeta.Namespace &&
            addr.ObjectMeta.Name.startsWith(
              l.Spec.Connection.Spec.AddressSpace.ObjectMeta.Name + "."
            )
        )
